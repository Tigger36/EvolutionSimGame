...
|-----------|---------------------------|-------------|-----|-------|-------------------|
| **Persistence** | `SavedSimulation` Codable; single-slot `PersistedRun` envelope; `RunPersistenceService`; Continue UI; save/restore and corrupt-save tests in core/app targets | **Partial** | Interactive relaunch/lifecycle smoke still needed for final beta confidence | 9 | Save/restore tests green + relaunch smoke |
